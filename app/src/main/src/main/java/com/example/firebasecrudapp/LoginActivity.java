...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        if (user != null) {
            SharedPreferences prefs = getApplicationContext().getSharedPreferences("isAdmin", MODE_PRIVATE);
            isAdmin = prefs.getBoolean("isAdmin", false);

            //if the user is not null then we are opening a main activity on below line.
            Intent i = new Intent(LoginActivity.this, MainActivity.class).putExtra("isAdmin", isAdmin);
            startActivity(i);
            this.finish();
        }
